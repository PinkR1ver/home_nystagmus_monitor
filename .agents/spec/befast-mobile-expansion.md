...
Face task and Silero VAD were downloaded to the external model cache, hashed, and not bundled. Face archive CRC passed. Android implementation is now active in its separate checkout on `codex/android-befast`. Session persistence and self-report validation compile and pass 4 targeted tests; phone inference and F/A/S UI are not yet connected. SenseVoice has a custom model license; openSMILE commercial use requires a separate license. Weight/source/version and clinical validation must remain distinct.
